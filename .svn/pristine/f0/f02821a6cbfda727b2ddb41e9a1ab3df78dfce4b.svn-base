...
/**
 * Created by Pleng on 27/04/2016.
 */
...
import java.util.StringTokenizer;
    ...
    public final class FirewallAPI
    {
        ...
        private static boolean applyIptablesRulesImpl(Context ctx, List<Integer> uidsWifi, List<Integer> uids3g, boolean showErrors) {
            ...
            final String customScript = ctx.getSharedPreferences(FirewallAPI.PREFS_NAME, 0).getString(FirewallAPI.PREF_CUSTOMSCRIPT, "");
            ...
            try {
                ...
                if (showErrors && code != 0) {
                    ...
                    DEBUGMANAGER.Log(DEBUGMANAGER.LOG_LEVEL_ERROR,"DroidWall", msg);
        ...
        public static boolean purgeIptables(Context ctx, boolean showErrors) {
            ...
            try {
                ...
                final String customScript = ctx.getSharedPreferences(FirewallAPI.PREFS_NAME, 0).getString(FirewallAPI.PREF_CUSTOMSCRIPT2, "");
                final StringBuilder script = new StringBuilder();
                script.append(scriptHeader(ctx));
                script.append("" +
        ...
        //CLAY
        public static File getArmV5Binary(Context _c)
        {
            return new File(_c.getDir("bin",0), "iptables_armv5");
        }
        public static File getBusyBoxBinary(Context _c)
        {
            return new File(_c.getDir("bin",0), "busybox_g1");
        }
        ...
        public static boolean assertBinaries(Context ctx, boolean showErrors) {
            ...
            try {
                ...
                File file = getArmV5Binary(ctx);
                if (!file.exists() || file.length()!=198652) {
                    copyRawFile(ctx, R.raw.iptables_armv5, file, "755");
                    changed = true;
                }
                // Check busybox
                file = getBusyBoxBinary(ctx);
                if (!file.exists()) {
                    copyRawFile(ctx, R.raw.busybox_g1, file, "755");
                    changed = true;
                }
                if (changed) {
                    Toast.makeText(ctx, R.string.toast_iptables_bin_installed, Toast.LENGTH_LONG).show();
                }
            } catch (Exception e) {
        ...
        public static void applicationRemoved(Context ctx, int uid) {
            ...
            if (savedUids_wifi.length() > 0) {
                ...
                while (tok.hasMoreTokens()) {
                    ...
                    if (uid_str.equals(token)) {
                        DEBUGMANAGER.Log(DEBUGMANAGER.LOG_LEVEL_WARN,"DroidWall", "Removing UID " + token + " from the wi-fi list (package removed)!");
                        changed = true;
                    } else {
            ...
            if (savedUids_3g.length() > 0) {
                ...
                while (tok.hasMoreTokens()) {
                    final String token = tok.nextToken();
                    if (uid_str.equals(token)) {
                        DEBUGMANAGER.Log(DEBUGMANAGER.LOG_LEVEL_WARN,"DroidWall", "Removing UID " + token + " from the 3G list (package removed)!");
                        changed = true;
                    } else {
                        if (newuids.length() > 0) newuids.append('|');
                        newuids.append(token);
